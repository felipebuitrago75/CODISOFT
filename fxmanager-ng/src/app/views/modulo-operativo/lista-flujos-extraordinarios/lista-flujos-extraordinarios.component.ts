...
export class ListaFlujosExtraordinariosComponent extends BaseCrudSearchComponent<any, any> implements OnInit {
  ...
  public onLoad(beanFilter: any, first: number, max: number, doFinish?: any) {
    this.flujoExtraordinarioService.getFlujosExtraordinarios(first, max).subscribe(flujosExtraordinarios => {
      let newRows = flujosExtraordinarios;
  ...
  public buscarConFiltro() {
    ...
    if (this.fechaFiltro) {
      flujoFiltro.fecha = this.fechaFiltro;
    }
